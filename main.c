...
#define DEVICE_NAME                     "Mercury Patch"                           	/**< Name of device. Will be included in the advertising data. */
#define NUS_SERVICE_UUID_TYPE           BLE_UUID_TYPE_VENDOR_BEGIN                  /**< UUID type for the Nordic UART Service (vendor specific). */

#define APP_ADV_INTERVAL                64                                          /**< The advertising interval (in units of 0.625 ms. This value corresponds to 40 ms). */
#define APP_ADV_TIMEOUT_IN_SECONDS      0                                           /**< 180 origThe advertising timeout (in units of seconds). */

#define APP_TIMER_PRESCALER             0                                           /**< Value of the RTC1 PRESCALER register. */
#define APP_TIMER_OP_QUEUE_SIZE         4                                           /**< Size of timer operation queues. */

#define MIN_CONN_INTERVAL               MSEC_TO_UNITS(20, UNIT_1_25_MS)             /**< Minimum acceptable connection interval (20 ms), Connection interval uses 1.25 ms units. */
#define MAX_CONN_INTERVAL               MSEC_TO_UNITS(125, UNIT_1_25_MS)             /**< 75 orig Maximum acceptable connection interval (75 ms), Connection interval uses 1.25 ms units. */
#define SLAVE_LATENCY                   4                                           /**< Slave latency. */
#define CONN_SUP_TIMEOUT                MSEC_TO_UNITS(10000, UNIT_10_MS)             /**< Connection supervisory timeout (6 seconds), Supervision Timeout uses 10 ms units. */
...
static void nus_data_handler(ble_nus_t * p_nus, uint8_t * p_data, uint16_t length)
{
    ...
    while (app_uart_put('\r') != NRF_SUCCESS); /** Check if this effects uart*/
    while (app_uart_put('\n') != NRF_SUCCESS);
}
/**@snippet [Handling the data received over BLE] */
...
static void uart_init(void)
{
    ...
    const app_uart_comm_params_t comm_params =
    {
        ...
        RTS_PIN_NUMBER,
        CTS_PIN_NUMBER,
        APP_UART_FLOW_CONTROL_DISABLED, /** Flow control enabled in example. */
        false,
...
/** Begin SAADC Functions.*/
void timer_handler(nrf_timer_event_t event_type, void * p_context)
{

}

/**SAADC Sampling event.*/
...
/**Copied from the SDK 11 version with UART BLE parsing*/
    ...
    if (p_event->type == NRF_DRV_SAADC_EVT_DONE)
    {
       ...
       //printf("ADC event number: %d\r\n",(int)m_adc_evt_counter);
        for (int i = 0; i < SAMPLES_IN_BUFFER; i++)
        {
           ...
           //SEGGER_RTT_printf(0, "%d\r ", p_event->data.done.p_buffer[i]);
        ...
        if((SAMPLES_IN_BUFFER*2) <= 20) 
        {
            bytes_to_send = (SAMPLES_IN_BUFFER*2);
        }
        else 
        {
            bytes_to_send = 20;
